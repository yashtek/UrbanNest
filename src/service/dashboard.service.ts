  ...
  async getDashboard(businessId: string, query: DashboardQuery) {
    ...
    const [rentRows, roomRows, tenantRows, staffRows, expenseRows] =
      await Promise.all([
          ...
          { $match: { businessId: businessObjectId } },
          {
            $group: {
              ...
              electricityTotal: {
                $sum: {
                  $cond: [
                    { $and: [{ $gte: ["$readingDate", startDate] }, { $lt: ["$readingDate", endDate] }] },
                    "$amount",
                    0,
                  ],
                },
        ...
        expenses().aggregate<{
          ...
          {
            $facet: {
              summary: [{ $group: { _id: null, total: { $sum: "$amount" }, count: { $sum: 1 } } }],
              byCategory: [
    ...
    return {
      ...
      rooms: {
        ...
        vacant: Math.max(0, (room?.totalCapacity ?? 0) - (room?.occupiedBeds ?? 0)),
      },
      tenants: {
