...
import {
  ...
  type ICommonOption,
  type OptionType,
} from "../modals/commonOption.modal";

const parseType = (type: string): OptionType => {
  if (!OPTION_TYPES.includes(type as OptionType))
    throw new AppError("Invalid option type", 400);
...
class CommonOptionService {
  ...
  async create(typeInput: string, nameInput: string) {
    ...
    try {
      await commonOptions().insertOne(option);
    } catch {
  ...
  async update(id: string, data: { name?: string; isActive?: boolean }) {
    ...
    try {
      const option = await commonOptions().findOneAndUpdate(
        { _id: new ObjectId(id) },
        { $set: fields },
  ...
  async require(id: string, type: OptionType) {
    ...
    const option = await commonOptions().findOne({
      _id: new ObjectId(id),
      type,
      isActive: true,
    });
  ...
  async populate<T extends Record<string, any>>(rows: T[], fields: string[]) {
    const ids = [
      ...new Set(
        rows
          .flatMap((row) =>
            fields.map((field) => row[field]).filter(ObjectId.isValid),
          )
          .map(String),
      ),
    ];
    const options = await commonOptions()
      .find(
        ...
        { projection: { type: 1, code: 1, name: 1, isActive: 1 } },
      )
      .toArray();
    const map = new Map(
      options.map((option) => [option._id.toString(), option]),
    );
    return rows.map((row) => {
      const result: Record<string, any> = { ...row };
      for (const field of fields)
        if (row[field]) result[field] = map.get(String(row[field])) ?? null;
      return result;
    }) as T[];
  }
}
