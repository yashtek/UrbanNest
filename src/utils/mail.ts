...
export async function sendPasswordResetEmail(
  ...
  const transporter = nodemailer.createTransport({
    ...
    auth: Bun.env.SMTP_USER
      ? { user: Bun.env.SMTP_USER, pass: Bun.env.SMTP_PASSWORD }
      : undefined,
  });
  await transporter.sendMail({
    from,
    to: email,
    subject: "Reset your UManage password",
    text: `Use this link to reset your password. It expires in 24 hours: ${url.toString()}`,
  });
}
