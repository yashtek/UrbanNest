...
mock.module("../db/db", () => ({
  getDB: () => ({
    collection: (name: string) => {
      if (name === "signup_otp_locks")
        return {
          findOneAndUpdate: async () => ({}),
          updateOne: async () => ({}),
        };
      if (name === "signup_msg91_used_tokens")
        return {
          insertOne: async (row: any) => {
            if (used.has(row._id)) throw { code: 11000 };
            used.add(row._id);
          },
        };
      return {
        ...
        updateOne: async (filter: any, update: any) => {
          ...
          for (const [key, amount] of Object.entries(update.$inc ?? {}))
            row[key] = (row[key] ?? 0) + Number(amount);
...
const { sendMsg91SignupOtp, verifyMsg91SignupOtp } =
  await import("./msg91-signup.service");
const savedFetch = globalThis.fetch;
const savedKey = process.env.MSG91_AUTH_KEY,
  savedTemplate = process.env.MSG91_TEMP_ID;
...
beforeEach(() => {
  ...
  delete process.env.MSG91_TEMP_ID;
  globalThis.fetch = (async (url: any, init: any) => {
    ...
    return Response.json(
      reject
        ...
        : {
            type: "success",
            message: String(url).includes("/otp/verify")
              ? "OTP verified success"
              : "3567736c53515771356c4878",
          },
    );
...
afterEach(() => {
  ...
  if (savedKey === undefined) delete process.env.MSG91_AUTH_KEY;
  else process.env.MSG91_AUTH_KEY = savedKey;
  if (savedTemplate === undefined) delete process.env.MSG91_TEMP_ID;
  else process.env.MSG91_TEMP_ID = savedTemplate;
});
...
test("send works without template configuration and uses only authkey", async () => {
  ...
  expect(new Headers(calls[0].init.headers).get("authkey")).toBe(
    "test-authkey",
  );
...
test("invalid OTP does not mark verified and attempts are limited", async () => {
  ...
  for (let i = 0; i < 5; i++)
    await expect(
      verifyMsg91SignupOtp(phone, { otp: "123456" }),
...
test("resend cooldown preserves attempt limits", async () => {
  await sendMsg91SignupOtp(phone);
  await expect(sendMsg91SignupOtp(phone)).rejects.toMatchObject({
    statusCode: 429,
  });
  rows.get(phone).lastSentAt = new Date(Date.now() - 31000);
  rows.get(phone).attempts = 4;
  await sendMsg91SignupOtp(phone);
