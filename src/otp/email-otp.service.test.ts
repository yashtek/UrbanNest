...
let sent:any, fail=false;
function matches(row:any,filter:any) {
 return Object.entries(filter).every(([key,value]:any)=>{
  if(value && typeof value==='object' && !(value instanceof Date)) return Object.entries(value).every(([op,v]:any)=>op==='$gt'?row[key]>v:op==='$lt'?row[key]<v:op==='$lte'?row[key]<=v:false);
  return row[key]===value;
 });
}
const table=(name:string)=>{if(!collections.has(name))collections.set(name,new Map());return collections.get(name)!;};
mock.module('../db/db',()=>({getDB:()=>({collection:(name:string)=>{
 const rows=table(name);
 const find=(f:any)=>[...rows.values()].find(r=>matches(r,f));
 return {
  createIndex:async()=>'',
  deleteOne:async(f:any)=>{const r=find(f);if(r)rows.delete(r._id);},
  findOneAndDelete:async(f:any)=>{const r=find(f);if(r)rows.delete(r._id);return r??null;},
  insertOne:async(r:any)=>{if(rows.has(r._id))throw {code:11000};rows.set(r._id,{...r});},
  replaceOne:async(f:any,r:any)=>{rows.set(f._id,{_id:f._id,...r});},
  updateOne:async(f:any,u:any)=>{const r=find(f);if(!r)return {modifiedCount:0};Object.assign(r,u.$set);return {modifiedCount:1};},
  findOneAndUpdate:async(f:any,u:any,o:any)=>{
   let r=find(f);
   if(!r && o?.upsert){if(rows.has(f._id))throw {code:11000};r={_id:f._id,...u.$setOnInsert};rows.set(f._id,r);}
   if(!r)return null;
   Object.assign(r,u.$set);for(const [k,v] of Object.entries(u.$inc??{}))r[k]=(r[k]??0)+Number(v);
   return {...r};
  },
 };
}})}));
...
test('sends random six-digit code to signup email and stores only its hash for five minutes',async()=>{
 ...
 expect(sent.to).toEqual([email]);expect(code()).toMatch(/^[1-9]\d{5}$/);
...
test('verification deletes OTP and returns single-use email-bound signup proof',async()=>{
 ...
 await expect(verifySignupEmailOtp(email,code())).rejects.toMatchObject({statusCode:400});
 await expect(consumeSignupEmailProof('other@example.com',result.verificationToken)).rejects.toMatchObject({statusCode:403});
...
test('expired codes fail even before MongoDB TTL cleanup',async()=>{
 await sendSignupEmailOtp(email);table('signup_email_otps').get(email).expiresAt=new Date(0);
 await expect(verifySignupEmailOtp(email,code())).rejects.toMatchObject({statusCode:400});
});
test('five wrong attempts exhaust code; cooldown prevents immediate resend',async()=>{
 await sendSignupEmailOtp(email);
 await expect(sendSignupEmailOtp(email)).rejects.toMatchObject({statusCode:429});
 for(let i=0;i<5;i++)await expect(verifySignupEmailOtp(email,'000000')).rejects.toMatchObject({statusCode:400});
 await expect(verifySignupEmailOtp(email,code())).rejects.toMatchObject({statusCode:400});
});
