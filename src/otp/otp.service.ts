...
const records = (): Collection<OtpRecord> =>
  getDB().collection<OtpRecord>("otp_requests");
...
const record = (phoneNumber: string, purpose: OtpPurpose) =>
  records().findOne({ phoneNumber, purpose });

export const ensureOtpIndexes = () =>
...
export const otpService = {
  async send(phoneNumber: string, purpose: OtpPurpose) {
    ...
    const inWindow =
      current &&
      time.getTime() - current.rateWindowStartedAt.getTime() < WINDOW_MS;
    if (current?.verifiedAt && !current.resetUsedAt)
      throw new AppError("OTP has already been verified", 409);
    if (inWindow && current!.sendCount >= MAX_SENDS)
    ...
    await records().updateOne(
      { phoneNumber, purpose },
      {
        $set: {
          ...
          updatedAt: time,
          rateWindowStartedAt: inWindow ? current!.rateWindowStartedAt : time,
        },
        $unset: { consumedAt: "", verifiedAt: "", resetUsedAt: "" },
        $setOnInsert: { phoneNumber, purpose },
      },
      { upsert: true },
    );
  ...
  async verify(phoneNumber: string, purpose: OtpPurpose, code: string) {
    ...
    if (
      ...
      !(await bcrypt.compare(code, current.codeHash))
    ) {
  ...
  async requireVerified(phoneNumber: string, purpose: OtpPurpose) {
    ...
    if (
      !current?.verifiedAt ||
      current.resetUsedAt ||
      Date.now() - current.verifiedAt.getTime() > VERIFIED_TTL_MS
    )
      throw new AppError("Phone verification is required or has expired", 403);
  },
  async consumeVerified(phoneNumber: string, purpose: OtpPurpose) {
    await records().updateOne(
      { phoneNumber, purpose },
      { $set: { resetUsedAt: new Date(), updatedAt: new Date() } },
    );
  },
};
