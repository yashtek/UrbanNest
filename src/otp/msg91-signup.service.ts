...
const config = (name: string) => {
  ...
  if (!value || value.startsWith("<"))
    throw new AppError(`Configure ${name} for signup OTP`, 503);
  return value;
};
const requests = () => getDB().collection("otp_requests");
const scope = (phoneNumber: string) => ({ phoneNumber, purpose: "signup" });
const invalid = () =>
  new AppError("Invalid or expired OTP. Request a new OTP if needed.", 400);
...
async function locked<T>(
  ...
  const locks = getDB().collection<{ _id: string; until: Date; owner: string }>(
    "signup_otp_locks",
  );
  const owner = crypto.randomUUID();
  try {
    const lock = await locks.findOneAndUpdate(
      { _id: phoneNumber, until: { $lte: new Date() } },
      { $set: { until: new Date(Date.now() + 60_000), owner } },
      { upsert: true, returnDocument: "after" },
    );
    if (!lock) throw new AppError("OTP request in progress. Please wait.", 429);
  } catch (error: any) {
    if (error?.code === 11000)
      throw new AppError("OTP request in progress. Please wait.", 429);
    throw error;
  }
  try {
    return await work();
...
export async function sendOtpRequest(
  ...
  try {
    ...
    throw new AppError("MSG91 is unavailable. Please try again later.", 503);
  }
  if (response.status === 429)
    throw new AppError("MSG91 rate limit reached. Try again later.", 429);
  ...
  try {
    ...
    throw new AppError("Unexpected MSG91 response", 502);
  }
...
export const sendMsg91SignupOtp = (phoneNumber: string) =>
  locked(phoneNumber, async () => {
    const current = await requests().findOne(scope(phoneNumber));
    const now = new Date();
    if (current?.verifiedAt && !current.resetUsedAt && current.expiresAt > now)
      throw new AppError("Phone already verified. Complete signup.", 409);
    const inWindow =
      current?.rateWindowStartedAt &&
      Date.now() - current.rateWindowStartedAt.getTime() < 900_000;
    if (
      (inWindow && current.sendCount >= 3) ||
      (current?.lastSentAt &&
        Date.now() - current.lastSentAt.getTime() < 30_000)
    )
      throw new AppError(
        "Wait 30 seconds before resending. Maximum 3 sends per 15 minutes.",
        429,
      );
    ...
    await requests().updateOne(
      scope(phoneNumber),
      {
        $set: {
          ...
          sendCount: inWindow ? current.sendCount + 1 : 1,
          lastSentAt: now,
          expiresAt: new Date(windowStart.getTime() + 900_000),
        },
      },
      { upsert: true },
    );
    ...
    const requestId =
      ...
      /^[a-zA-Z0-9_-]{16,128}$/.test(candidate) &&
      candidate !== process.env.MSG91_AUTH_KEY
        ? candidate
        : null;

    await requests().updateOne(scope(phoneNumber), {
      $set: {
        provider: "msg91-template",
        providerRequestId: requestId,
        deliveryStatus: "unknown",
        otpExpiresAt: new Date(Date.now() + 300_000),
        attempts: inWindow ? (current.attempts ?? 0) : 0,
        updatedAt: new Date(),
      },
      $unset: {
        codeHash: "",
...
export const verifyMsg91SignupOtp = (
  phoneNumber: string,
  input: { otp: string },
) =>
  locked(phoneNumber, async () => {
    const current = await requests().findOne(scope(phoneNumber));
    if (
      current?.provider !== "msg91-template" ||
      current.consumedAt ||
      !current.otpExpiresAt ||
      current.otpExpiresAt <= new Date()
    )
      throw invalid();
